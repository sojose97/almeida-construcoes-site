...
const renderWithPromotion=()=>{
  ...
  $('#grid').innerHTML=list.map(p=>`<article class="card" data-id="${esc(p.id)}"><a class="card-open" href="produto.html?id=${encodeURIComponent(p.id)}">${p.imagem?`<img class="card-image processing" data-clean-image src="${esc(p.imagem)}" alt="${esc(p.nome)}">`:`<div class="photo">${esc(p.marca?.[0]||'')}</div>`}<div class="card-content"><small>${esc(String(p.marca||'').toUpperCase())} · ${esc(p.grupo)}</small><h3>${esc(p.nome)}</h3><div class="variant">${p.variacoes.length>1?`${p.variacoes.length} variações`:(p.variacoes[0].nome==='Padrão'?'':esc(p.variacoes[0].nome))}</div><div class="card-price">${cardPrice(p,0)}</div></div></a><div class="quick-buy"><label>Variação<select data-variant>${p.variacoes.map((v,index)=>`<option value="${index}">${esc(v.nome)}</option>`).join('')}</select></label><label>Qtd.<input data-quantity type="number" min="1" value="1" inputmode="numeric"></label><button data-add class="quick-add">Adicionar</button></div></article>`).join('')||'<p class="notice">Nenhum produto em promoção no momento.</p>';
