...
async function directStatus(orderId,status){
  const rows=await request('/rest/v1/orders?select=id,user_id,status,amount_due_cents,cashback_reserved_cents&id=eq.'+encodeURIComponent(orderId));
  ...
  const paid=Math.max(0,Number(order.amount_due_cents)||0),patch=status==='confirmed'?{status:'confirmed',amount_paid_cents:paid,cashback_credited_cents:Math.floor(paid*.02),confirmed_at:new Date().toISOString()}:{status:'cancelled',cancelled_at:new Date().toISOString()};
  const changed=await request('/rest/v1/orders?id=eq.'+encodeURIComponent(order.id)+'&status=eq.pending',{method:'PATCH',headers:{'Content-Type':'application/json',Prefer:'return=representation'},body:JSON.stringify(patch)});
